...
This is normal HTML code
	<?php
	$checkin_date =  date('d-m-y', strtotime($_POST['checkin']));
	$checkout_date =  date('d-m-y', strtotime($_POST['checkout']));
	echo "<br>".$checkin_date."<br>";
	echo $checkout_date;
	include './auth.php';
	
	//execute the SQL query and return records
	$result = mysqli_query($dbhandle, "select * from Room where room.room_id in 
		  (
	...
	while ($row = mysqli_fetch_array($result)) {
	   echo "<br>room_id:".$row{'room_id'}." Room_type:".$row{'bed_type'}."Occupany: ".$row{'occupancy'}."<br>"; 
	   //display the results
		print "<div class=\"row\">\n";
